...
public:
	static inline const SaveLoad description[] = {
		 ...
		 * from before a field existed simply don't have it and the in-class
		 * default applies on load. Which is what lets the old, single
		 * decouple_count be carried alongside the two fields that replaced it
		 * and told apart from them without a version number: an old save has
		 * only the old field, and a new one always writes it as zero. See
