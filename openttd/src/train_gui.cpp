...

	{
		...
		for (; v != nullptr && (rtl ? px > 0 : px < max_width); v = backwards ? v->Previous() : v->Next()) {
			...
			if (rtl ? px + width > 0 : px - width < max_width) {
				...
				seq.Draw(px + (rtl ? -offset.x : offset.x), y + offset.y, pal, v->vehstatus.Test(VehState::Crashed) || v->IsWrecked());
			}
		...
		for (; v != nullptr && vscroll_pos > -vscroll_cap; v = v->GetNextVehicle()) {
			...
			do {
				...
				if (vscroll_pos <= 0 && vscroll_pos > -vscroll_cap) {
					...
					PaletteID pal = (v->vehstatus.Test(VehState::Crashed) || v->IsWrecked()) ? PALETTE_CRASH : GetVehiclePalette(u);
					VehicleSpriteSeq seq;
					u->GetImage(dir, EngineImageType::InDetails, &seq);
					seq.Draw(px + (rtl ? -offset.x : offset.x), r.top - line_height * vscroll_pos + sprite_y_offset + pitch, pal, v->vehstatus.Test(VehState::Crashed) || v->IsWrecked());
				}
				px += rtl ? -width : width;
				dx += width;
				u = u->Next();
