...
enum class OrderBoardMode : uint8_t {
	...
	TrainToNext,    ///< A train whose orders name the station of the next order.
...
enum ModifyOrderFlags : uint8_t {
	...
	MOF_DECOUPLE,        ///< Toggle whether this order leaves part of the train behind.
	MOF_DECOUPLE_COUNT,  ///< Change how many wagons stay on with the engine when decoupling; the engine is never counted, so zero keeps none of them.
	...
	MOF_BOARD_MODE,      ///< Passes an OrderBoardMode: how a road vehicle gets itself carried on from this station (see road_on_rail.h).
	...
	MOF_COUPLE_BUY,      ///< Passes the EngineID of the wagon this order buys when the depot is short of them, or EngineID::Invalid() to stop buying.
	MOF_END
};
