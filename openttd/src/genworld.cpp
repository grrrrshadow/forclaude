...
void LoadTownData()
{
	...
	for (auto &feature : town_data) {
		...
		for (auto tile : SpiralTileSequence(target_tile, 16, 0, 0)) {
			std::tuple<CommandCost, Money, TownID> result = Command<Commands::FoundTown>::Do(DoCommandFlag::Execute, tile, TownSize::Small, is_city, _settings_game.economy.town_layout, false, 0, name, 0);
